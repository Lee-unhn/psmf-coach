...
- **階段自動升階**：隨體重下降自動從 ATTACK → CRUISE → STABILIZE → MAINTAIN，**熱量/碳水隨之往上帶**（門檻相對你的目標體重）+ 訓練區塊隨階段調整。
- **每日醫學新知**：每天菜單 email 附一則當週權威論文（同儕審查來源，每天輪不同篇）。
- **動態每日菜單**：依「前一日回填 + 近期趨勢」決定隔日菜單，用食材庫算出每餐 macros + 價格；高階段自動補碳水/脂肪到目標。
